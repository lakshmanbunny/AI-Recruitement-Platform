...
# Navigate to the 'exp' root directory
if "backend" in script_dir:
    # exp/backend/core/rag_evaluation -> exp/
    project_root = os.path.dirname(os.path.dirname(os.path.dirname(script_dir)))
else:
    # exp/core/rag_evaluation -> exp/
    project_root = os.path.dirname(os.path.dirname(script_dir))

backend_dir = os.path.join(project_root, "backend")

# PRIORITIZE backend_dir to avoid picking up legacy root-level packages
sys.path.insert(0, backend_dir)   # For 'app' and 'core' inside backend
sys.path.append(project_root)    # For 'config' in the root
...
from backend.core.rag_evaluation.llm_rag_judge import EnterpriseLLMRAGJudge
